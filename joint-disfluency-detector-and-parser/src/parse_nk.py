...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, attn_mask=None):
        ...
        if attn_mask is not None:
            ...
            # Ensure mask is boolean type for PyTorch compatibility
    ...
    def parse_batch(self, sentences, golds=None, return_label_scores_charts=False):
        ...
        if self.char_encoder is not None:
            ...
            if self.encoder is not None:
                # Explicitly convert mask to boolean type for PyTorch compatibility
                mask = all_word_end_mask.unsqueeze(-1).expand(-1, -1, features.shape[-1]).bool()
                features_packed = features.masked_select(mask).reshape(-1, features.shape[-1])
